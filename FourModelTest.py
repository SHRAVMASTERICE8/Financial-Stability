...
algos = {
    'gradBoost': GradientBoostingRegressor(n_estimators=2),
    'elasticNet': linear_model.ElasticNet(alpha=0.5),
    'randForest': RandomForestRegressor(n_estimators=1),
...
#     i = 0
#     for dep in depRange:
#         print('Working dep=%s' % dep)
#
#         error = complexErrors(algos, X, y, trainDepth=dep, predictionDepth=90, frequency=300)
#         result[0, i] = dep
#         result[1, i] = error['elasticNet']
#         result[2, i] = error['gradBoost']
#         result[3, i] = error['randForest']
#         result[4, i] = error['neuralMLP']
#         i = i + 1
#
#     return result
#
...
# print(result)
#
# # print(result[:, np.where(result[1, :] == result[1, :].min())])