...
def algosPlot(algos, ticks):
    ...
    for algo in algos:
        ...
        if algo == 'Volatility':
            ...
            plt.plot(range(0, len(pred)), pred, label=algo, linewidth=1)
    plt.plot(range(0, len(pred)), np.zeros(len(pred)), linestyle='--', linewidth=0.5)
    plt.xticks(range(0, len(ticks))[::1200], ticks[::1200])
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Volatility (Standardized)', fontsize=12)
    plt.title('Overfitting in Randomized Forests Regression')
...
data = pullAndMerge('SP500', 'TWEXB', 'CPIHOSNS', 'A191RL1Q225SBEA', 'PSAVERT', 'T10YFF', 'DGS10', 'BAMLH0A0HYM2',
                    'T10Y2Y', 'FEDFUNDS', 'USROA', 'USROE', 'USSTHPI', 'STLFSI', 'NCBCMDPMVCE', 'MPRIME',
                    'CILACBQ158SBOG', 'INTDSRUSM193N', 'TERMCBAUTO48NS', 'TOTLL', 'BAMLH0A0HYM2EY', 'BAMLC0A0CM',
                    'RU3000VTR', 'TOTBKCR')
