...
pred = np.concatenate([np.zeros(90), est.predict(X)])
...
xVals = [x, x, x[:2481]]
yVals = [np.zeros(len(x)), pred, norm]
labels = ['ref', 'Gradient Boosting Model', 'Volatility']
...
# xVals = [x, x, x, x, x]
# yVals = [y, volt*10, voltAv*10, est.predict(X)*75+400, norm*75+400]
# labels = ['S&P 500 (Market Value)', 'Volatility (Residual of 150 Day Regression)', 'Volatility (100 Day Moving Average)', 'Gradient Boosting Model', 'norm']
# bigPlot(xVals, yVals, labels, sp500['DATE'], 'S&P 500 Volatility\nMeasured as the Predictive Error of 150 Day Regressions')
